...
export function ErrorBoundedTable() {
  ...
  return (
    ...
    >
      <ErrorBoundary
        resetKeys={[orgUnits, program, periods]}
...
function Table({ height }: { height: number }) {
      ...
      <EmptyList
        message={
          dimensionsNotSelected
            ? i18n.t(
                "Select intervention, organisation unit, and period to start"
              )
            : i18n.t(
                "There are no results for the selected intervention, period, and organisation unit"
              )
        }
      />
    )
  ...
  return (
    <div
      style={{ padding: "0 16px", flex: 1, margin: "32px 0" }}
      className=" w-100"
    >
      <CustomDataTable
