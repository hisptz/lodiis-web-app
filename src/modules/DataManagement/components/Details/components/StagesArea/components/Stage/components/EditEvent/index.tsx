...
export function EditEventModal({
                                   ...
                                   hide,
                                   stage,
                               }: { event?: DHIS2Event; onClose: () => void; hide: boolean; stage: ProgramStageConfig & ProgramStage; }) {
    const {refresh} = useProfileData();
    const fields = useMemo(() => {
            if (!event) {
                return [];
            }
            return compact(stage.view.filter(({editable}) => editable).map(({field}) => field));
        }
        , [event, stage]);

    const {show, hide: hideModal} = useAlert(({message}) => message, ({type}) => ({...type, duration: 3000}))

    function getDefaultValues(fields: RHFDHIS2FormFieldProps[], event?: DHIS2Event) {
        return fromPairs(fields.map((field) => [field.name, getDataElementValue(event?.dataValues ?? [], field.name)]))
    }
    ...
    const [updateEvent, {loading}] = useDataMutation(eventMutation, {
        onComplete: () => {
            show({message: i18n.t("Service updated successfully"), type: {success: true}});
            refresh();
            onClose();
        },
        onError: (error) => {
    ...
    const onEditClick = async (data: Record<string, any>) => {
        ...
        function updateEventDataValues(event: DHIS2Event, data: Record<string, any>) {
            ...
            <ModalContent>
                <FormProvider {...form}>
                    <div style={{display: "grid", gap: 8, gridTemplateColumns: "1fr"}}>
                        {
                            fields?.map(field => (<RHFDHIS2FormField key={field?.name} {...field}  />))
                        }
                    </div>
                </FormProvider>
            </ModalContent>
            <ModalActions>
                <ButtonStrip>
                    ...
                    <Button loading={loading} primary
                            onClick={form.handleSubmit(onEditClick)}> {loading ? `${i18n.t("Updating")}...` : i18n.t("Update")}</Button>
                </ButtonStrip>
            </ModalActions>
        </Modal>
    )
