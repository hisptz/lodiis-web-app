...
import { fromPairs, sortBy, head } from "lodash";
...
const columns = [
  {
    ...
    get: (event: DHIS2Event) => {
      ...
      return (
        <Tag positive={isComplete}>{isComplete ? "Completed" : "Active"}</Tag>
      );
    },
  },
  {
...
function getFormattedDate(date: string): string {
  const formattedDate = DateTime.fromISO(date).diffNow("days").negate();
  const { days } = formattedDate;
  return days >= 1
    ? formattedDate.rescale().toFormat("dd 'days' ago")
    : i18n.t("today");
}
...
export function Stage({
  ...
  const [deleteEvent] = useDataMutation(eventDeleteMutation, {
    onComplete: () => {
      show({
        message: i18n.t("Event deleted successfully"),
        type: { success: true },
      });
      refresh();
    },
  ...
  const rows = useMemo(
    () =>
      events.map((event) => ({
        ...fromPairs(
          sanitizedColumns.map((column) => [column.key, column.get(event)])
        ),
        id: event.event,
      })),
    [events]
  ...
  const eventLastUpdatedDate = useMemo(() => {
    const latestEvent = head(
      sortBy(events, ({ lastUpdated }) =>
        DateTime.fromISO(lastUpdated ?? "").toMillis()
      ).reverse()
    );

    return latestEvent?.lastUpdated ?? "";
  }, [events]);
            ...
            <div className="row space-between">
              <div style={{ alignItems: "flex-end" }} className="row gap-16">
                ...
                {eventLastUpdatedDate ? (
                  <p style={{ margin: 0 }}>
                    {i18n.t("Last updated")}{" "}
                    {getFormattedDate(eventLastUpdatedDate)}
                  </p>
                ) : null}
              </div>
