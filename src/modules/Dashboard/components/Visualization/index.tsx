...
function getDataValueFromAnalytics(id: string, data?: Analytics) {
    ...
    return reduce(filter(data.rows, (row) => get(row, dataIndex) === id), (acc, value) => {
...
function resolveVisualization(type: "chart" | "table" | "map" | "single-value", {options, data, height, width}: {
    ...
    switch (type) {
        ...
        function getItemValue({id, label, percentage}: { id: string; label: string; percentage?: boolean }) {
            ...
            return <SingleValueContainer title={""} singleValueItems={sanitizedSingleValueItems}/>
    }
}
