...
function resolveVisualization(
	...
	switch (type) {
		case "chart":
			...
			return <ChartAnalytics config={config} analytics={data} />;
		case "table":
			if (!data) {
				return null;
			}
		...
		case "single-value":
			...
			const totalValue = {
				...totalValueConfig,
				value:
					getDataValueFromAnalytics(totalValueConfig.id, data) ?? 1,
			};
			...
			const sanitizedSingleValueItems =
				singleValueItems.map(getItemValue);
