...
export function DashboardList() {
    ...
    return (
        <Box height="48px">
            <Card>
                <div style={{margin: "auto"}} className="row align-center gap-8 h-100">
                    {
                        programs?.map((program: ProgramConfig) => (
                            <Chip onClick={onChipClick(program.id)} selected={programId === program.id}
                                  key={`${program.id}-dashboard-chip`}>{program.name}</Chip>))
                    }
                </div>
            </Card>
        </Box>
